...
export default function AuditReview() {
  ...
  const applyFilters = () => {
    ...
    if (filters.procedureCode) {
      filtered = filtered.filter((p) =>
        p.procedure_code
          ?.toLowerCase()
          .includes(filters.procedureCode.toLowerCase())
      );
    }
    if (filters.status !== "all") {
      filtered = filtered.filter(
        (p) => p.evaluation_result?.approval_status === filters.status
      );
    }
  ...
  const getStatsCards = () => {
    ...
    const approved = procedures.filter(
      (p) => p.evaluation_result?.approval_status === "מאושר"
    ).length;
    const pending = procedures.filter(
      (p) => p.evaluation_result?.approval_status === "דרוש בדיקה נוספת"
    ).length;
    const rejected = procedures.filter(
      (p) => p.evaluation_result?.approval_status === "נדחה"
    ).length;
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className="text-slate-600 mt-1">
                בדיקה וניתוח של פרוצדורות שבוצעו בעבר
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-1">
            <DateRangeFilter
              ...
              proceduresCount={
                filteredProcedures.filter((p) => p.xray_url).length
              }
            />

            {isAuditing && <AuditProgress progress={auditProgress} />}
          </div>
